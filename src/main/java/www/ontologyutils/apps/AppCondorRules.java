...
public class AppCondorRules {
	...
	public AppCondorRules(String ontologyFilePath) {
		...
		ontology = Annotate.newOntology(ontologyFilePath);
	...
	private static Set<OWLAxiom> superNormalize(OWLAxiom a) {
		...
		while (true) {
			...
			if (leftConj.size() == 2) {
				assert(!iter.hasNext());
				OWLAxiom axiom = new OWLSubClassOfAxiomImpl(newConj, right, Annotate.getAxiomAnnotations(a));
				res.add(axiom);
				return res;
			}
			...
			OWLAxiom axiom = new OWLSubClassOfAxiomImpl(newConj, newAtom, Annotate.getAxiomAnnotations(a));
			res.add(axiom);
		}
	}
	...
	public static void main(String[] args) {		
		...
		try {
			...
			rgc.getMapAxioms().entrySet().stream()
			.forEach(e -> {
				try {
